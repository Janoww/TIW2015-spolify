...
/**
 * A registry to hold and provide access to Album objects globally. This acts as
...
public class AlbumRegistry {
	...
	/**
	 * Initializes the registry with a collection of albums. This method should only
	...
	public static synchronized void initialize(Collection<Album> initialAlbums) {
		...
		if (initialAlbums != null) {
			...
			albumMap.putAll(initialAlbums.stream().collect(Collectors.toMap(Album::getIdAlbum, Function.identity())));
		}
		initialized = true;
	...
	/**
	 * Retrieves all albums stored in the registry. Returns an unmodifiable view of
	...
	/**
	 * Adds a new album to the registry. If an album with the same ID already
