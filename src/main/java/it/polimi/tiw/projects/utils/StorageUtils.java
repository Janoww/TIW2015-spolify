...
public class StorageUtils {
    ...
    /**
     * Validates a filename, resolves it against a base storage directory, and ensures the resulting
     * path is securely within that directory. Also verifies file existence.
     *
     * @param filename The filename to validate and resolve.
     * @param baseStorageDirectory The base directory against which the filename is resolved.
     * @return The validated, normalized, and real (existent) Path object.
     * @throws DAOException If the file is not found, or if the resolved path is outside the base
     *         storage directory (indicating an access attempt violation).
     * @throws IllegalArgumentException If the filename is null, blank, contains invalid path
     *         characters, or results in an invalid path.
     */
    public static Path validateAndResolveSecurePath(String filename, Path baseStorageDirectory)
            throws DAOException, IllegalArgumentException {

        log.debug("Validating and resolving secure path for filename: {} in base directory: {}",
                filename, baseStorageDirectory);
        ...
        if (filename.contains("/") || filename.contains("\\") || filename.contains("..")) {
            log.warn("Validation failed: Filename '{}' contains invalid path characters.",
                    filename);
        ...
        try {
            ...
            if (!fileRealPath.startsWith(baseStorageRealPath)) {
                ...
                throw new DAOException(
                        "Access denied: Filename resolves outside storage directory.",
            ...
            log.error("SecurityException during path validation for filename '{}' in {}: {}",
                    filename, baseStorageDirectory, e.getMessage(), e);
            throw new DAOException(
