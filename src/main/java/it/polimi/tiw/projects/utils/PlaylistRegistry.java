...
/**
 * A registry to hold and provide access to Playlist objects globally.
...
public class PlaylistRegistry {
    ...
    /**
     * Initializes the registry with a collection of playlists.
    ...
    public static synchronized void initialize(Collection<Playlist> initialPlaylists) {
        ...
        if (initialPlaylists != null) {
            ...
            playlistMap.putAll(initialPlaylists.stream()
                    .collect(Collectors.toMap(Playlist::getIdPlaylist, Function.identity())));
        }
        initialized = true;
    ...
    /**
     * Retrieves all playlists stored in the registry.
    ...
    /**
     * Adds a new playlist to the registry.
     * If a playlist with the same ID already exists, an exception is thrown.
     *
     * @param playlist The Playlist object to add.
     * @return {@code true} if the playlist was added successfully, {@code false} if
     *         the
     *         playlist is null or a playlist with the same ID already exists.
     * @throws IllegalStateException if the registry has not been initialized.
     */
